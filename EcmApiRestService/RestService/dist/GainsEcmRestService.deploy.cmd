...
set _DeploySetParametersFile=%RootPath%GainsEcmRestService.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%GainsEcmRestService.zip" (
echo "%RootPath%GainsEcmRestService.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%GainsEcmRestService.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\x86\\Debug\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='GainsEcmRestService\\App_Data$'
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%GainsEcmRestService.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\x86\\Debug\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='GainsEcmRestService\\App_Data$' -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%GainsEcmRestService.deploy-readme.txt" (
...
start notepad "%RootPath%GainsEcmRestService.deploy-readme.txt"
)
echo =========================================================
goto :eof
